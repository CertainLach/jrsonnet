...
/** \file This file is a library interface for evaluating Jsonnet. It is written
 * in Rust but exposes a C interface for easier wrapping by other languages. See
 * \see jsonnet_lib_test.c for an example of using the library.
 */

/** The version string of th Jsonnet interpreter.
 *
 * This is currently grepped out of this file by setup.py, Makefile, and
 * CMakeLists.txt so be aware of that when making changes.
 *
...
/** Return the version string of the Jsonnet interpreter.  Conforms to semantic
 * versioning https://semver.org/ If this does not match LIB_JSONNET_VERSION
 ...
 *
 * The returned char* should be allocated with jsonnet_realloc.  It will be
 * cleaned up by libjsonnet when no-longer needed.
 *
 ...
 * \param found_here Set this byref param to path to the file, absolute or
 * relative to the process's CWD.  This is necessary so that imports from the
 * content of the imported file can be resolved correctly.  Allocate memory with
 * jsonnet_realloc.  Only use when *success = 1. \param success Set this byref
 * param to 1 to indicate success and 0 for failure. \param buf Set this byref
 * param to the content of the imported file, or an error message.  Allocate
 * memory with jsonnet_realloc.  Do not include a null terminator byte. \param
 * buflen Set this byref param to the length of the data returned in buf.
 * \returns 0 to indicate success and 1 for failure.  On success, the content is
 * in *buf.  On failure, an error message is in *buf.
 */
typedef int JsonnetImportCallback(void *ctx, const char *base, const char *rel,
                                  char **found_here, char **buf,
                                  size_t *buflen);

/** An opaque type which can only be utilized via the jsonnet_json_* family of
 * functions.
 */
...
/** If the value is a number, return 1 and store the number in out, otherwise
 * return 0.
 */
 ...
 *
 * This is useful if you want to abort with an error mid-way through building a
 * complex value.
 */
 ...
 *
 * The returned JsonnetJsonValue* should be allocated with jsonnet_realloc.  It
 * will be cleaned up along with the objects rooted at argv by libjsonnet when
 * no-longer needed.  Return a string upon failure, which will appear in Jsonnet
 * as an error.  The argv pointer is an array whose size matches the array of
 * parameters supplied when the native callback was originally registered.
 *
 * \param ctx User pointer, given in jsonnet_native_callback.
 * \param argv Array of arguments from Jsonnet code.
 * \param success Set this byref param to 1 to indicate success and 0 for
 * failure. \returns The content of the imported file, or an error message.
 */
typedef struct JsonnetJsonValue *
JsonnetNativeCallback(void *ctx, const struct JsonnetJsonValue *const *argv,
                      int *success);

/** Allocate, resize, or free a buffer.  This will abort if the memory cannot be
 * allocated.  It will only return NULL if sz was zero.
 *
 * \param buf If NULL, allocate a new buffer.  If an previously allocated
 * buffer, resize it. \param sz The size of the buffer to return.  If zero,
 * frees the buffer. \returns The new buffer.
 */
 ...
 *
 * This will appear in Jsonnet as a function type and can be accessed from
 * std.nativeExt("foo").
 *
 * DO NOT register native callbacks with side-effects!  Jsonnet is a lazy
 * functional language and will call your function when you least expect it,
 * more times than you expect, or not at all.
 *
 * \param vm The vm.
 ...
 * \param params NULL-terminated array of the names of the params.  Must be
 * valid identifiers.
 */
...
/** Evaluate a file containing Jsonnet code, return a number of named JSON
 * files.
 *
 * The returned character buffer contains an even number of strings, the
 * filename and JSON for each JSON file interleaved.  It should be cleaned up
 * with jsonnet_realloc.
 *
 * \param filename Path to a file containing Jsonnet code.
 * \param error Return by reference whether or not there was an error.
 * \returns Either the error, or a sequence of strings separated by \0,
 * terminated with \0\0.
 */
char *jsonnet_evaluate_file_multi(struct JsonnetVm *vm, const char *filename,
                                  int *error);

/** Evaluate a string containing Jsonnet code, return a number of named JSON
 * files.
 *
 * The returned character buffer contains an even number of strings, the
 * filename and JSON for each JSON file interleaved.  It should be cleaned up
 ...
 *
 * The returned character buffer contains several strings.  It should be cleaned
 * up with jsonnet_realloc.
 *
 * \param filename Path to a file containing Jsonnet code.
 * \param error Return by reference whether or not there was an error.
 * \returns Either the error, or a sequence of strings separated by \0,
