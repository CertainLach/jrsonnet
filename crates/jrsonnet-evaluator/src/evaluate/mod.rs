...
use jrsonnet_parser::{
	...
	ForSpecData, IfSpecData, IndexPart, LiteralType, LocExpr, Member, ObjBody, ParamsDesc,
	SliceDesc,
};
use jrsonnet_types::ValType;

use self::destructure::{destruct_lazy, destruct_strict};
use crate::{
	arr::ArrValue,
	bail,
	...
	evaluate::{
		self,
		operator::{evaluate_add_op, evaluate_binary_op_special, evaluate_unary_op},
	},
...
pub fn evaluate_comp_strict(
	...
	match specs.first() {
		...
		Some(CompSpec::IfSpec(IfSpecData(cond))) => {
			if bool::from_untyped(evaluate(ctx, cond)?)? {
				evaluate_comp_strict(ctx, &specs[1..], callback)?;
			}
		}
		Some(CompSpec::ForSpec(ForSpecData(var, expr))) => match evaluate(ctx, expr)? {
			Val::Arr(list) => {
		...
		Some(CompSpec::ForSpec(ForSpecData(var, expr))) => match evaluate(ctx, expr)? {
			Val::Arr(list) => {
				for item in list.iter_lazy() {
					...
					let mut new_bindings = BindingsMap::with_capacity(var.capacity_hint());
					destruct_lazy(var, item, fctx.clone(), &mut new_bindings)?;
	...
	impl Unbound for UnboundLocals {
		...
		fn bind(&self, sup_this: SupThis) -> Result<Context> {
			...
			let mut new_bindings =
				BindingsMap::with_capacity(self.locals.iter().map(BindSpec::capacity_hint).sum());
			...
			let ctx = ctx
				.with_bindings_sup_this(new_bindings, sup_this)
				.into_future(fctx);
...
pub fn evaluate_member_list_object(ctx: &Context, members: &[Member]) -> Result<ObjValue> {
	let count_members = members
		.iter()
		.filter(|v| matches!(v, Member::Field(_)))
		.count();
	...
	for member in members {
		match member {
			Member::Field(field) => {
				evaluate_field_member(&mut builder, ctx, uctx.clone(), field)?;
			}
...
pub fn evaluate_object(ctx: &Context, object: &ObjBody) -> Result<ObjValue> {
	Ok(match object {
		...
		ObjBody::ObjComp(obj) => {
			...
			evaluate_comp_lazy(ctx, &obj.compspecs, &mut |ctx| {
...
fn evaluate_index_single(
	indexable: Val,
	index: Val,
	#[cfg(feature = "exp-null-coaelse")] null_coaelse: bool,
) -> Result<Val> {
	#[cfg(not(feature = "exp-null-coaelse"))]
	let null_coaelse = false;

	Ok(match (indexable, index) {
		(Val::Obj(v), Val::Str(key)) => match v.get(key.clone().into_flat())? {
			...
			None => {
				...
				bail!(NoSuchField(key.into_flat(), suggestions));
			}
		},
		(i @ (Val::Str(_) | Val::Arr(_)), Val::Num(n)) => {
			let n = n.get_index()?;
			let i = i.into_indexable().expect("is indexable");
			if null_coaelse {
				i.index(n)?.unwrap_or(Val::Null)
			} else {
				i.try_index(n)?
			}
		}

		(indexable @ (Val::Arr(_) | Val::Str(_) | Val::Obj(_)), n) => {
			bail!(ValueIndexMustBeTypeGot(
				indexable.value_type(),
				match indexable {
					Val::Arr(_) | Val::Str(_) => ValType::Num,
					Val::Obj(_) => ValType::Str,
					_ => unreachable!(),
				},
				n.value_type(),
			))
...
#[allow(clippy::too_many_lines)]
fn evaluate_index(ctx: &Context, indexable: &LocExpr, parts: &[IndexPart]) -> Result<Val> {
	...
	let mut indexable = if matches!(indexable.expr(), Expr::Literal(LiteralType::Super)) {
		...
		let name = evaluate(ctx, &part.value)?;
		...
		evaluate(ctx, indexable)?
	};

	for part in parts {
		#[cfg(feature = "exp-null-coaelse")]
		if indexable.is_null() && part.null_coaelse {
			return Ok(Val::Null);
		}
		let index = evaluate(ctx, &part.value)?;
		indexable = evaluate_index_single(
			indexable,
			index.clone(),
			#[cfg(feature = "exp-null-coaelse")]
			part.null_coaelse,
		)
		.with_description_src(&part.value, || match index {
			Val::Str(s) => {
				format!("field <{s}> access")
			}
			Val::Num(i) => {
				format!("index <{i}> access")
			}
			_ => "invalid index access".to_string(),
		})?;
	}
	Ok(indexable)
}
...
fn evaluate_slice(
	...
	let indexable = evaluate(ctx, value)?;
	...
	Indexable::into_untyped(indexable.into_indexable()?.slice(start, end, step)?)
}
...
pub fn evaluate(ctx: &Context, expr: &LocExpr) -> Result<Val> {
	...
	Ok(match expr.expr() {
		...
		Index { indexable, parts } => {
			ensure_sufficient_stack(|| evaluate_index(ctx, indexable, parts))?
		}
		LocalExpr(bindings, returned) => {
			let mut new_bindings =
				BindingsMap::with_capacity(bindings.iter().map(BindSpec::capacity_hint).sum());
			let fctx = Context::new_future();
			for b in bindings {
				evaluate_dest(b, fctx.clone(), &mut new_bindings)?;
			}
		...
		ArrComp(expr, comp_specs) => error_slow_path(
			|| {
				let mut out = Vec::new();
				let mut ctx = ctx.clone();
				evaluate_comp_strict(&mut ctx, comp_specs, &mut |ctx| {
			...
			|| {
				let mut out = Vec::new();
				evaluate_comp_lazy(ctx, comp_specs, &mut |ctx| {
					let expr = expr.clone();
					let ctx = ctx.clone();
		...
		IfElse {
			...
			if in_frame(
				...
				|| bool::from_untyped(evaluate(ctx, &cond.0)?),
			)? {
		...
		Slice(value, desc) => evaluate_slice(ctx, CallLocation::new(&loc), value, desc)?,
